...
export const ServiceNode = memo(({ id, data, selected }: NodeProps) => {
    ...
    // Icon Selection based on node type and label
    let ServiceIcon = Server
    const lowerLabel = nodeData.label.toLowerCase()

    // Service nodes can have different icons based on their role
    if (lowerLabel.includes('api') || lowerLabel.includes('gateway')) {
        ServiceIcon = Globe
    } else if (lowerLabel.includes('cpu') || lowerLabel.includes('compute')) {
        ServiceIcon = Cpu
    } else {
        ServiceIcon = Server // Default for services
    }
